...
[Galois Counter mode](https://en.wikipedia.org/wiki/Galois/Counter_Mode) is the most widely used block cipher mode in TLS today. It's an "authenticated encryption with associated data" cipher mode ([**AEAD**](https://en.wikipedia.org/wiki/Authenticated_encryption)).

For each encryption, it produces a tag that can be used to verify the integrity of the message.

![GCM Mode](./assets/GCM_encryption.svg)

## Attacks

### Forbidden attack

```python {linenos=table,filename="forbidden_attack.py"}
...
- [Key-Recovery Attacks on GCM with Repeated Nonces](https://toadstyle.org/cryptopals/63.txt)
